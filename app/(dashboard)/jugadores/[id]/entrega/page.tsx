...
import { checkUploadable, fileKind, FOLDERS, folderLabel, formatBytes, type Folder } from '@/lib/utils/players';
...
export default function NewDeliveryPage() {
  ...
  const deliver = async () => {
    ...
    if (failed.length === 0) {
      toast.success(`${pending.length === 1 ? 'Archivo entregado' : `${pending.length} archivos entregados`} a ${player.given_name}`);
      router.push(`/jugadores/${player.id}/${folder}`);
      return;
    }
    setPending(failed);
    toast.error(`${failed.length} de ${pending.length} no se pudieron subir. Quedan en la lista para reintentar.`);
  };
    ...
    >
      ...
      {player && (
        <div className="space-y-6">
          <Section label="Destino" hint="En qué carpeta y con qué nombre lo verá" padded={false}>
            <div className="flex min-h-14 items-center justify-between gap-3 px-4 py-2">
              <span className="text-[15px]">Carpeta</span>
              <Select value={folder} onValueChange={(v) => setFolder(v as Folder)} disabled={uploading}>
          ...
          <Section label="Archivos" hint="Se entregan tal cual, sin recomprimir">
            <div className="space-y-3">
              ...
              {pending.length > 0 && (
                <ul className="overflow-hidden rounded-xl bg-background">
                  {pending.map((item, i) => {
                    const Icon = fileKind(item.file.type, item.file.name) === 'video' ? Video : ImageIcon;
                    return (
                      <li key={item.key}>
                        ...
                        <div className="flex items-center gap-3.5 px-4 py-3">
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="block truncate text-[15px] font-medium">{item.file.name}</span>
                            <span className={cn('font-mono tabular text-xs', item.error ? 'text-destructive' : 'text-muted-foreground')}>
          ...
          <Button onClick={deliver} disabled={uploading || pending.length === 0} size="lg" className="h-11 w-full">
            {uploading ? (
              <>
